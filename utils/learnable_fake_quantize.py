...
class _LearnableFakeQuantize(nn.Module):
    ...
    * attr: `learning_enabled` defines the flag for enabling backpropagation for scale and zero point.
    ...
    def __init__(self, observer, quant_min=0, quant_max=255, scale=1., zero_point=0., channel_len=-1,
                 use_grad_scaling=True, weight=False, **observer_kwargs):
    ...
    def enable_fakequant(self):
        r"""Enable Fakequant with fixed rounded zero_point
        """
        self.toggle_qparam_learning(enabled=True) \
            .toggle_fake_quant(enabled=True) \
            .toggle_observer_update(enabled=False)
        self.zero_point.requires_grad = False
        self.zero_point.data.copy_(torch.clip(self.zero_point.data.round(), self.quant_min, self.quant_max))

    @torch.jit.export
    def enable_fixed_estimate(self):
        self.toggle_qparam_learning(enabled=False) \
            .toggle_fake_quant(enabled=True) \
            .toggle_observer_update(enabled=False)
    ...
    def calculate_qparams(self):
        if self.static_enabled[0] == 1:
            ...
            return self.scale.data, self.zero_point.data.round() 
    ...
    def forward(self, X):
        ...
        if self.init and self.wt:
            ...
            self.zero_point = Parameter(torch.FloatTensor([0.] * len(_zero_point)))
            ...
            if self.learning_enabled[0] == 1:
                if self.use_grad_scaling:
                    if self.wt:
                        ...
                        grad_factor = 1.0 / (X[0].numel() * self.quant_max) ** 0.5
                else:
                    grad_factor = 1.0
                if self.qscheme in (
...
def enable_fixedfake(mod):
    if type(mod) == _LearnableFakeQuantize:
        mod.enable_fakequant()

def enable_fixed_estimate(mod):
    if type(mod) == _LearnableFakeQuantize:
        mod.enable_fixed_estimate()