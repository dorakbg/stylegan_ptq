...
def get_q_config(q_params):
    """Returns PyTorch qconfig given quantization parameters dict

    Args:
        q_params (dict): quantization parameters

    Returns:
        [qconfig]: PyTorch qconfig
    """
...
def get_static_quant_model(model_path, q_params={}, numits_calib=500, get_fp=False, disable_observer=True):
    """Configers quantized model and calibrates it on data

    Args:
        model_path (str): path to full-precion model weights
        q_params (dict, optional): Quantization parameters. Defaults to {}.
        numits_calib (int, optional): Number of iterations of calibtration. Defaults to 500.
        get_fp (bool, optional): Return full-precion model?. Defaults to False.
        disable_observer (bool, optional): Disable observer at the end of calibtration?. Defaults to True.

    Returns:
        [nn.Module]: quantized calibrated model
    """
...
def get_fid_from_qparams(q_params, data_dir, model_path, reference_data, qfid=True, use_cache=True):
    """Computes FID and qFID score given quantization parameters

    Args:
        q_params (dic): quantization parameters
        data_dir (str): path to directory where intermediate data will be stored
        model_path (str): path to full-precion
        reference_data (str): data to compare against, must be path to data generated by full precion model in case qfid==True
                              or else path to real data 
...
def transform_style(generator):
    """Transforms style-network of generator into blocked structure

    Args:
        generator (nn.Module): StyleGAN generator
    """
    style = []
...
def get_pretrained_model(path, fp_path, q_params, get_fp=False):
    """Returns configured pretrained model

    Args:
        path (str): path to pretrained quantized model
        fp_path (str): path to full-precion model
        q_params (dict): quantization parameters
        get_fp (bool, optional): Get full-precion model?. Defaults to False.
