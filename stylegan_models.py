...
class PixelNorm(nn.Module):
    ...
    def forward(self, input):
        return self.quant(input / torch.sqrt(torch.mean(input ** 2, dim=1, keepdim=True) + 1e-8))
...
class Blur(nn.Module):
    ...
    def forward(self, input):
        return self.quant(blur(input, self.weight, self.weight_flip))
...
class AdaptiveInstanceNorm(nn.Module):
    ...
    def forward(self, input, style):
        ...
        out = self.norm(input)
        out = self.add.add(self.mul.mul(gamma, out), beta)
...
class NoiseInjection(nn.Module):
    ...
    def forward(self, image, noise):
        return self.add.add(image, self.mul.mul(self.weight, noise))
        ...
        if noise is None:
            ...
            for i in range(step + 1):
                ...
                for b in range(batch):
                    if (self.training): # fix seed for per layer noise
                        torch.manual_seed(round(input[0][b].detach().abs().sum().data.cpu().item() * 100))
                    n.append(torch.randn(1, size, size, device=input[0].device))
                n = torch.stack(n)
                noise.append(self.quant_noise(n))
        else:
            noise = [self.quant_noise(n) for n in noise]
...
class Discriminator(nn.Module):
    ...
    def forward(self, input, step=5, alpha=1):
